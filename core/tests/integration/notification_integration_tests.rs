...
mod tests {
    ...
    async fn test_notification_system_integration() {
        ...
        assert!(results.success_rate() >= 100.0, "All notification tests must pass");
    }
    ...
    async fn test_multi_channel_notification(
        ...
        let notification_request = NotificationRequest {
            ...
            html_message: Some("<p>A <strong>high-risk</strong> remediation requires your approval</p>".to_string()),
            data: std::collections::HashMap::from([
                ("approval_id".to_string(), serde_json::Value::String("test-approval-123".to_string())),
                ("resource_type".to_string(), serde_json::Value::String("Storage Account".to_string())),
            ]),
        ...
        if result.successful_deliveries < 2 {
            return Err(format!("Expected at least 2 successful deliveries, got {}", result.successful_deliveries));
        }
    ...
    async fn test_template_processing(
        ...
        let notification_request = NotificationRequest {
            ...
            data: std::collections::HashMap::from([
                ("approver_name".to_string(), serde_json::Value::String("John Approver".to_string())),
                ("resource_type".to_string(), serde_json::Value::String("Storage Account".to_string())),
                ("resource_name".to_string(), serde_json::Value::String("productionstg123".to_string())),
                ("risk_level".to_string(), serde_json::Value::String("high".to_string())),
                ("requested_by".to_string(), serde_json::Value::String("alice@company.com".to_string())),
            ]),
        ...
        if result.successful_deliveries != 1 {
            return Err(format!("Template notification failed: {} deliveries", result.successful_deliveries));
        }
        ...
        if !last_notification.processed_subject.contains("Storage Account productionstg123") {
    ...
    async fn test_rate_limiting(
        ...
        notification_system.add_channel(rate_limited_channel).await?;
        ...
        println!("    📊 Rate limiting test: {} sent, {} rate limited", sent_count, rate_limited_count);
    ...
    async fn test_retry_mechanism(
        ...
        println!("    🔄 Retry test: {} retry attempts made", notification_entry.retry_count);
    ...
    async fn test_notification_history(
        ...
        for i in 0..3 {
            let request = NotificationRequest {
                ...
                html_message: None,
                data: std::collections::HashMap::from([
                    ("remediation_id".to_string(), serde_json::Value::String(format!("rem-{}", i))),
                ]),
                channels: vec![],
                scheduled_at: None,
                expires_at: None,
        ...
        if history.len() < 3 {
            return Err(format!("Expected at least 3 history entries, got {}", history.len()));
        }
        
        // Test filtering by event type
        let filtered_history: Vec<_> = history.into_iter()
            .filter(|entry| entry.event_type == NotificationEventType::RemediationCompleted)
            .collect();
        
        if filtered_history.len() < 3 {
            return Err(format!("Expected 3 RemediationCompleted entries, got {}", filtered_history.len()));
        }
        ...
        println!("    📈 Statistics: {} total notifications, {:.1}% success rate", 
...
impl NotificationSystem {
    pub async fn initialize_default_channels(&self) -> Result<(), String> {
        ...
        let teams_channel = NotificationChannel {
            ...
            config: ChannelConfig {
                teams_webhook_url: Some("https://company.webhook.office.com/webhookb2/test".to_string()),
    ...
    pub async fn get_notification_statistics(&self) -> Result<NotificationStatistics, String> {
        ...
        let successful = history.iter().filter(|h| h.status == NotificationStatus::Sent).count();
