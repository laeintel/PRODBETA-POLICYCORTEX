...
mod tests {
    ...
    async fn test_performance_benchmarks() {
        ...
        match test_concurrent_remediation_performance().await {
            Ok(metrics) => {
                ...
                println!(
                    "    📊 Metrics: {} req/sec, avg latency: {}ms",
        ...
        match test_bulk_remediation_scalability().await {
            Ok(metrics) => {
                ...
                println!(
                    "    📊 Metrics: {} items/sec, {} parallel workers",
        ...
        match test_notification_throughput().await {
            Ok(metrics) => {
                ...
                println!(
                    "    📊 Metrics: {} notifications/sec, {} channels",
        ...
        match test_memory_usage_under_load().await {
            Ok(metrics) => {
                ...
                println!(
                    "    📊 Metrics: peak memory: {}MB, memory efficiency: {:.2}%",
        ...
        match test_system_responsiveness().await {
            Ok(metrics) => {
                ...
                println!(
                    "    📊 Metrics: p95 latency: {}ms, error rate: {:.2}%",
        ...
        assert!(
            ...
            "Performance tests indicate system issues"
        );
    }
    ...
    async fn test_concurrent_remediation_performance() -> Result<PerformanceMetrics, String> {
        ...
        if requests_per_second < 10.0 {
            return Err(format!(
                "Low throughput: {:.2} req/sec (expected >= 10)",
                requests_per_second
            ));
        }
        ...
        if average_latency_ms > 1000 {
            return Err(format!(
                "High latency: {}ms (expected <= 1000ms)",
                average_latency_ms
            ));
        }
    ...
    async fn test_bulk_remediation_scalability() -> Result<BulkPerformanceMetrics, String> {
        ...
        for batch_size in batch_sizes {
            ...
            println!(
                "    📊 Batch size {}: {} items/sec",
                batch_size, items_per_second
            );
        }
        ...
        if best_throughput < 20 {
            return Err(format!(
                "Low bulk throughput: {} items/sec (expected >= 20)",
                best_throughput
            ));
        }
    ...
    async fn test_notification_throughput() -> Result<NotificationPerformanceMetrics, String> {
        ...
        if notifications_per_second < 30 {
            return Err(format!(
                "Low notification throughput: {} notifications/sec (expected >= 30)",
    ...
    async fn test_memory_usage_under_load() -> Result<MemoryMetrics, String> {
        ...
        if memory_growth > 500 {
            return Err(format!(
                "Excessive memory growth: {}MB (expected <= 500MB)",
                memory_growth
            ));
        }
        ...
        if memory_efficiency < 70.0 {
            return Err(format!(
                "Poor memory efficiency: {:.1}% (expected >= 70%)",
                memory_efficiency
            ));
        }
    ...
    async fn test_system_responsiveness() -> Result<ResponsivenessMetrics, String> {
        ...
        if p95_latency > 2000 {
            return Err(format!(
                "High P95 latency: {}ms (expected <= 2000ms)",
                p95_latency
            ));
        }
...
impl MockWorkflowEngine {
    ...
    ) -> Result<super::remediation_workflow_tests::BulkRemediationResult, String> {
...
use policycortex_core::remediation::notification_system::{
    NotificationEventType, NotificationPriority, NotificationRecipient, NotificationRequest,
    NotificationResult, NotificationStatus, RecipientType,
};
