...
impl PersistentAuditChain {
    ...
    pub async fn add_entry_persistent(
        ...
        sqlx::query(query)
            ...
            .bind(serde_json::to_value(&entry.event_type)?)
            .bind(serde_json::to_value(&entry.actor)?)
            .bind(serde_json::to_value(&entry.resource)?)
            .bind(&entry.action)
            .bind(serde_json::to_value(&entry.outcome)?)
