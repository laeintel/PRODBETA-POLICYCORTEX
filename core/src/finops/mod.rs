...
pub trait FinOpsEngine: Send + Sync {
    ...
    async fn execute_optimization(
        &self,
        optimization_id: &str,
    ) -> Result<OptimizationResult, FinOpsError>;
}
...
impl AzureFinOpsEngine {
    ...
    async fn generate_optimal_schedule(
        &self,
        _resource_id: &str,
    ) -> Result<crate::azure_client_async::AutoShutdownSchedule, FinOpsError> {
    ...
    async fn generate_idle_recommendation(
        &self,
    ...
    fn generate_idle_recommendation_json(
        &self,
        _resource: &serde_json::Value,
        _metrics: &serde_json::Value,
        monthly_cost: f64,
    ...
    fn assess_performance_impact(
        &self,
        _metrics: &serde_json::Value,
        _recommended_sku: &serde_json::Value,
    ) -> String {
        "Low".to_string()
    }
    ...
    fn calculate_real_savings(
        &self,
        pattern: &UsagePattern,
        pricing: &serde_json::Value,
        commitment_mix: &CommitmentMix,
    ) -> SavingsEstimate {
        let on_demand_rate = pricing
            .get("on_demand_rate")
            .and_then(|v| v.as_f64())
            .unwrap_or(200.0);
        let reserved_1y = pricing
            .get("reserved_1y_price")
            .and_then(|v| v.as_f64())
            .unwrap_or(150.0);
        let reserved_3y = pricing
            .get("reserved_3y_price")
            .and_then(|v| v.as_f64())
            .unwrap_or(120.0);

        let baseline_cost = pattern.avg_hours * on_demand_rate;
        let optimized_cost = (pattern.avg_hours * commitment_mix.three_year_percentage / 100.0
            * reserved_3y)
    ...
    fn calculate_commitment_confidence(&self, pattern: &UsagePattern) -> f64 {
        ...
        let stability =
            1.0 - ((pattern.p90_hours - pattern.p10_hours) / pattern.avg_hours).min(1.0);
    ...
    fn calculate_idle_score(&self, metrics: &serde_json::Value) -> f64 {
        ...
        let cpu_avg = metrics
            ...
            .and_then(|v| v.as_f64())
            .unwrap_or(50.0);
        let network_avg = metrics
            .get("network_io")
            .and_then(|v| v.as_f64())
            .unwrap_or(10.0);
        let disk_avg = metrics
            .get("disk_io")
            .and_then(|v| v.as_f64())
            .unwrap_or(5.0);
        let memory_avg = metrics
        ...
        cpu_idle * cpu_weight
            + network_idle * network_weight
            + disk_idle * disk_weight
            + memory_idle * memory_weight
    }
    ...
    fn calculate_rightsizing_from_json(
        &self,
        ...
        if cpu_headroom > (vcpus * 0.5) && memory_headroom > (memory_gb * 0.5) {
    ...
    async fn detect_cost_anomaly(&self, timeseries: &[CostDataPoint]) -> Vec<CostAnomaly> {
        ...
        for (i, point) in timeseries.iter().enumerate() {
            if i >= window_size {
                ...
                let variance = window.iter().map(|p| (p.cost - mean).powi(2)).sum::<f64>()
                    / window_size as f64;
    ...
    async fn infer_cause(&self, point: &CostDataPoint, history: &[CostDataPoint]) -> String {
        ...
        } else if point.data_transfer_gb
            > history.iter().map(|p| p.data_transfer_gb).sum::<f64>() / history.len() as f64 * 2.0
        {
    ...
    fn find_smaller_sku(&self, current: &VmSku) -> Option<VmSku> {
        ...
        let sku_families = vec![
            VmSku {
                name: "D2s_v3".to_string(),
                vcpus: 2,
                memory_gb: 8.0,
                cost_hourly: 0.096,
            },
            VmSku {
                name: "D4s_v3".to_string(),
                vcpus: 4,
                memory_gb: 16.0,
                cost_hourly: 0.192,
            },
            VmSku {
                name: "D8s_v3".to_string(),
                vcpus: 8,
                memory_gb: 32.0,
                cost_hourly: 0.384,
            },
            VmSku {
                name: "D16s_v3".to_string(),
                vcpus: 16,
                memory_gb: 64.0,
                cost_hourly: 0.768,
            },
        ...
        let current_name_prefix = if current.name.len() >= 2 {
            &current.name[..2]
        } else {
            "D"
        };
...
impl FinOpsEngine for AzureFinOpsEngine {
                ...
                == "Microsoft.Compute/virtualMachines"
            {
                ...
                let metrics = self
                    ...
                    .get_resource_metrics(
                        ...
                        vec!["Percentage CPU", "Network In Total", "Network Out Total"],
                    )
                    .await
                    .map_err(|e| FinOpsError::AzureError(e.to_string()))?;
                ...
                if idle_score > 0.7 {
                    ...
                    idle_resources.push(IdleResource {
                        ...
                        cpu_avg: metrics
                            ...
                            .and_then(|v| v.as_f64())
                            .unwrap_or(0.0),
                        network_io_avg: metrics
                            .get("network_average")
                            .and_then(|v| v.as_f64())
                            .unwrap_or(0.0),
                        recommendation: self.generate_idle_recommendation_json(
                            &resource,
                            &metrics,
                            cost_data
                                .get("monthly_cost")
    ...
    async fn analyze_rightsizing(&self) -> Result<Vec<RightsizingOpportunity>, FinOpsError> {
        ...
        for vm in vms {
            ...
            if let Some(current_sku) = vm.get("sku") {
                if let Some(recommended_sku) = self.calculate_rightsizing_from_json(
                    current_sku.as_object().unwrap_or(&serde_json::Map::new()),
                    &metrics,
                ) {
                    // Get REAL pricing from Azure
                    let current_sku_name = current_sku
                        .get("name")
                        .and_then(|v| v.as_str())
                        .unwrap_or("");
                    ...
                    let recommended_sku_name = recommended_sku
                        .get("name")
                        .and_then(|v| v.as_str())
                        .unwrap_or("");
                    let new_cost = self
                    ...
                    opportunities.push(RightsizingOpportunity {
                        ...
                        performance_impact: self.assess_performance_impact(
                            &metrics,
                            &serde_json::json!(recommended_sku),
                        ),
    ...
    async fn plan_commitments(&self) -> Result<Vec<CommitmentRecommendation>, FinOpsError> {
        ...
        for (family, pattern) in usage_by_family {
            ...
            if commitment_mix.one_year_percentage > 0.0
                || commitment_mix.three_year_percentage > 0.0
            {
                ...
                recommendations.push(CommitmentRecommendation {
                    ...
                    on_demand_rate: pricing
                        .get("on_demand_rate")
                        .and_then(|v| v.as_f64())
                        .unwrap_or(200.0),
                    commitment_type: "Savings Plan".to_string(),
                    term_length: if commitment_mix.three_year_percentage
                        > commitment_mix.one_year_percentage
                    {
                        "3 years".to_string()
                    } else {
                        "1 year".to_string()
                    },
                    recommended_coverage: commitment_mix.one_year_percentage
                        + commitment_mix.three_year_percentage,
                    commitment_rate: pricing
                        .get("savings_plan_rate")
                        .and_then(|v| v.as_f64())
                        .unwrap_or(150.0),
    ...
    async fn calculate_savings(&self) -> Result<SavingsSummary, FinOpsError> {
        ...
        let rightsizing_savings: f64 = rightsizing
            .as_ref()
            .map(|opps| opps.iter().map(|o| o.savings_monthly).sum())
            .unwrap_or(0.0);

        let commitment_savings: f64 = commitments
            .as_ref()
            .map(|recs| recs.iter().map(|c| c.monthly_savings).sum())
            .unwrap_or(0.0);
        ...
        let automated_count = idle
            .as_ref()
            .map(|r| {
                r.iter()
                    .filter(|i| i.recommendation.action == "deallocate")
                    .count()
            })
            .unwrap_or(0);
    ...
    async fn execute_optimization(
        ...
        match opt_type {
            ...
            _ => {
                return Err(FinOpsError::OptimizationError(format!(
