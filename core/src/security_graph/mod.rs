...
impl SecurityGraphEngine {
    ...
    pub async fn build_from_azure(&mut self, client: &crate::azure_client_async::AsyncAzureClient) -> Result<(), SecurityGraphError> {
        // Fetch identities
    ...
    async fn add_identities(&mut self, client: &crate::azure_client_async::AsyncAzureClient) -> Result<(), SecurityGraphError> {
        // Fetch REAL identities from Azure AD
        let identities = client.get_identities().await
            .map_err(|e| SecurityGraphError::AzureError(format!("Failed to fetch identities: {}", e)))?;
    ...
    async fn add_roles_and_permissions(&mut self, client: &crate::azure_client_async::AsyncAzureClient) -> Result<(), SecurityGraphError> {
        // Fetch REAL roles from Azure RBAC
        let role_definitions = client.get_role_definitions().await
            .map_err(|e| SecurityGraphError::AzureError(format!("Failed to fetch roles: {}", e)))?;
        
        for role_def in role_definitions {
            // Determine criticality based on permissions
            let permissions = role_def.get("permissions")
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str()).map(|s| s.to_string()).collect::<Vec<_>>())
                .unwrap_or_default();
            
            let criticality = if permissions.iter().any(|p| p.contains("*/delete") || p.contains("*/write")) {
                if permissions.iter().any(|p| p.contains("Microsoft.Authorization") || p.contains("Microsoft.KeyVault")) {
                    "Critical".to_string()
                } else {
                    "High".to_string()
                }
            ...
            let role = SecurityNode::Role {
                id: role_def.get("id").and_then(|v| v.as_str()).unwrap_or("unknown").to_string(),
                name: role_def.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown").to_string(),
                permissions,
                criticality,
            };
    ...
    async fn add_resources(&mut self, client: &crate::azure_client_async::AsyncAzureClient) -> Result<(), SecurityGraphError> {
        // Fetch REAL resources from Azure Resource Graph
        let resources = client.get_all_resources_with_health().await
            .map_err(|e| SecurityGraphError::AzureError(format!("Failed to fetch resources: {}", e)))?;
        ...
        for resource_data in resource_items {
            ...
            let resource = SecurityNode::Resource {
                ...
                name: resource_data.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown").to_string(),
    ...
    async fn add_network_topology(&mut self, client: &crate::azure_client_async::AsyncAzureClient) -> Result<(), SecurityGraphError> {
        // Fetch REAL network topology from Azure
        let network_data = client.get_network_topology().await
            .map_err(|e| SecurityGraphError::AzureError(format!("Failed to fetch network topology: {}", e)))?;
        
        // Add public endpoints
        let public_endpoints = network_data.get("public_endpoints").and_then(|v| v.as_array()).map(|v| v.clone()).unwrap_or_default();
        for public_endpoint in public_endpoints {
            let exposure = if public_endpoint.get("has_public_ip").and_then(|v| v.as_bool()).unwrap_or(false) {
                "Internet".to_string()
            } else if public_endpoint.get("is_reachable_from_internet").and_then(|v| v.as_bool()).unwrap_or(false) {
                "Public".to_string()
            } else {
                "Private".to_string()
            };
    ...
    async fn add_data_stores(&mut self, client: &crate::azure_client_async::AsyncAzureClient) -> Result<(), SecurityGraphError> {
        // Fetch REAL data stores from Azure
        let datastores = client.get_data_stores().await
            .map_err(|e| SecurityGraphError::AzureError(format!("Failed to fetch data stores: {}", e)))?;
    ...
    async fn build_relationships(&mut self, client: &crate::azure_client_async::AsyncAzureClient) -> Result<(), SecurityGraphError> {
        // Fetch REAL role assignments from Azure
        let role_assignments = client.get_role_assignments().await
        ...
        let network_flows = client.get_network_flows().await
    ...
    pub fn find_attack_paths(&self, target_sensitivity: &str) -> Vec<AttackPath> {
        ...
        for entry in &entry_points {
            for target in &critical_targets {
                ...
                let simple_paths: Vec<Vec<NodeIndex>> = all_simple_paths(&self.graph, *entry, *target, 0, Some(10))
                    .collect();
                
                for path_nodes in simple_paths {
                    let path = self.score_and_build_path(path_nodes);
                    if path.risk_score > 0.5 { // Only include significant paths
                        paths.push(path);
                    }
                }
            }
    ...
    fn score_and_build_path(&self, node_indices: Vec<NodeIndex>) -> AttackPath {
        ...
        for &idx in &node_indices {
            if let Some(node) = self.graph.node_weight(idx) {
                match node {
                    ...
                    SecurityNode::Resource { encryption_status, backup_status, .. } => {
        ...
        let risk_score = 
            privilege_score * self.scoring_weights.privilege_weight +
        ...
        let path_node_ids: Vec<String> = node_indices.iter()
            .filter_map(|&idx| {
    ...
    fn generate_mitigations(&self, path: &[NodeIndex]) -> Vec<MitigationBundle> {
        ...
        let encryption_controls = vec![
            SecurityControl {
                ...
                parameters: HashMap::from([
                    ("key_type".to_string(), "CustomerManagedKey".to_string()),
                ]),
            },
            SecurityControl {
    ...
    pub async fn apply_mitigation(&self, bundle: &MitigationBundle, client: &crate::azure_client_async::AsyncAzureClient) -> Result<MitigationResult, SecurityGraphError> {
    ...
    async fn apply_control(&self, control: &SecurityControl, _client: &crate::azure_client_async::AsyncAzureClient) -> Result<(), SecurityGraphError> {
        // Simulate control application
        tracing::info!("Applying {} control to {}", control.control_type, control.target);
...
pub async fn analyze_security_exposure(
    ...
    let azure_client = client
        .ok_or_else(|| SecurityGraphError::AzureError("Azure client not initialized. Please ensure Azure credentials are configured.".to_string()))?;
...
fn generate_prioritized_mitigations(paths: &[AttackPath]) -> Vec<MitigationBundle> {
    ...
    for path in paths {
        for bundle in &path.mitigation_bundles {
            mitigation_map.entry(bundle.name.clone())
                .or_insert_with(|| bundle.clone());
        }
    }
    
    let mut mitigations: Vec<MitigationBundle> = mitigation_map.into_values().collect();
    mitigations.sort_by(|a, b| b.effectiveness.partial_cmp(&a.effectiveness).unwrap());
