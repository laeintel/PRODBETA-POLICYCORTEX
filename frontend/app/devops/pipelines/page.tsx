  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400">Pipelines with drill-in run history</p>
      </div>
      ...
      <div className="bg-gray-900 rounded-lg border border-gray-800 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-800/50 border-b border-gray-800">
            <tr>
              <th className="text-left px-6 py-3 text-sm font-medium text-gray-400">Pipeline</th>
              <th className="text-left px-6 py-3 text-sm font-medium text-gray-400">Branch</th>
              <th className="text-left px-6 py-3 text-sm font-medium text-gray-400">Last Run</th>
          ...
          <tbody>
            {pipelines.map(p => (
              <tr key={p.name} className="border-b border-gray-800 hover:bg-gray-800/50">
                ...
                <td className="px-6 py-4 text-gray-400 text-sm">{p.branch}</td>
                <td className="px-6 py-4 text-gray-400 text-sm">{p.lastRun}</td>
        ...
        <div className="fixed right-0 top-0 h-full w-[520px] bg-gray-900 border-l border-gray-800 p-6 overflow-y-auto z-50">
          ...
          <div className="grid grid-cols-2 gap-3">
            {selected.runs.map(run => (
              <button key={run.id} onClick={() => setSelectedRun(run)} className="p-3 bg-gray-800/60 rounded hover:bg-gray-800 text-left">
                ...
                <p className="text-xs text-gray-400">{run.time} • {run.duration}</p>
              </button>
            ))}
          </div>
          ...
          {selectedRun && (
            <div className="mt-4 p-4 bg-gray-800/50 rounded">
              ...
              <p className="text-sm text-gray-400 mb-2">Status: {selectedRun.status} • Duration: {selectedRun.duration}</p>
...
function StatusBadge({ status }: { status: 'running'|'idle'|'failed' }) {
  const map = { running: 'text-blue-400', idle: 'text-gray-400', failed: 'text-red-400' }
  return <span className={`text-sm font-medium ${map[status]}`}>{status}</span>
}

function RunStatus({ status }: { status: 'success'|'failed'|'running' }) {
  if (status === 'success') return <span className="text-green-400 text-xs inline-flex items-center gap-1"><CheckCircle className="w-3 h-3"/> success</span>
  if (status === 'failed') return <span className="text-red-400 text-xs inline-flex items-center gap-1"><XCircle className="w-3 h-3"/> failed</span>
  return <span className="text-blue-400 text-xs inline-flex items-center gap-1"><Clock className="w-3 h-3"/> running</span>
}