      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-100/50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800">
            <tr>
              <th className="text-left px-6 py-3 text-sm font-medium text-gray-600 dark:text-gray-400">Build</th>
              ...
              <th className="text-left px-6 py-3 text-sm font-medium text-gray-600 dark:text-gray-400">Time</th>
              <th className="text-left px-6 py-3 text-sm font-medium text-gray-600 dark:text-gray-400">Action</th>
            </tr>
          </thead>
          <tbody>
            {builds.map(b => (
              <tr key={b.id} className="border-b border-gray-200 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800/50">
                ...
                <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{b.time}</td>
                <td className="px-6 py-4"><button type="button" className="text-xs px-3 py-1 bg-gray-200 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 rounded" onClick={() => setSelected(b)}>Details</button></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="fixed right-0 top-0 h-full w-[520px] bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 p-6 overflow-y-auto z-50">
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400">Branch: {selected.branch} • Status: {selected.status}</p>
            </div>
          ...
          <div className="space-y-3">
            <div className="text-xs text-gray-700 dark:text-gray-300 font-mono bg-gray-100 dark:bg-gray-900 p-3 rounded border border-gray-200 dark:border-gray-800">{`[build] install deps\n[build] run tests\n[build] compile\n[build] package`}</div>
            <div>
              <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Artifacts</p>
              {selected.artifacts.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">No artifacts</p>
              ) : (
