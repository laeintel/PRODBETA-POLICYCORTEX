...
class RBACManagementService:
    ...
    async def list_roles(
        self, subscription_id: str, scope: Optional[str] = None
    ) -> List[RBACResponse]:
        ...
        try:
                        ...
                        permissions=[
                            {
                                ...
                                "data_actions": (
                                    perm.data_actions if hasattr(perm, "data_actions") else []
                                ),
                                "not_data_actions": (
                                    perm.not_data_actions
                                    if hasattr(perm, "not_data_actions")
                                    else []
                                ),
                            }
            ...
            logger.info(
                "roles_listed", subscription_id=subscription_id, scope=scope, count=len(roles)
            )

            return roles

        except AzureError as e:
            logger.error("list_roles_failed", error=str(e), subscription_id=subscription_id)
            raise Exception(f"Failed to list roles: {str(e)}")

    async def get_role_definition(
        self, subscription_id: str, role_id: str, scope: Optional[str] = None
    ) -> RBACResponse:
        ...
        try:
            ...
            logger.info(
                "role_definition_retrieved", subscription_id=subscription_id, role_id=role_id
            )
                ...
                permissions=[
                    {
                        ...
                        "not_data_actions": (
                            perm.not_data_actions if hasattr(perm, "not_data_actions") else []
                        ),
                    }
                    for perm in role.permissions
                ],
            ...
            logger.error(
                "role_definition_not_found", subscription_id=subscription_id, role_id=role_id
            )
    ...
    async def list_role_assignments(
        self, subscription_id: str, principal_id: Optional[str] = None, scope: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # List role assignments
            assignment_list = client.role_assignments.list_for_scope(scope=scope, filter=filter_str)

            async for assignment in assignment_list:
                # Get role definition details
                role_def_id = assignment.role_definition_id.split("/")[-1]
                try:
                    role_def = await self.get_role_definition(subscription_id, role_def_id, scope)
                ...
                assignments.append(
                    {
                        ...
                        "created_on": (
                            assignment.created_on.isoformat() if assignment.created_on else None
                        ),
                        "updated_on": (
                            assignment.updated_on.isoformat() if assignment.updated_on else None
                        ),
            ...
            logger.error(
                "list_role_assignments_failed", error=str(e), subscription_id=subscription_id
            )
            raise Exception(f"Failed to list role assignments: {str(e)}")

    async def create_role_assignment(
        self, subscription_id: str, assignment_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def delete_role_assignment(self, subscription_id: str, assignment_id: str) -> None:
        ...
        try:
            ...
            await client.role_assignments.delete(
                scope=assignment["scope"], role_assignment_name=assignment_name
            )
    ...
    async def check_access(
        self, subscription_id: str, principal_id: str, action: str, scope: str
    ) -> bool:
        ...
        try:
            ...
            assignments = await self.list_role_assignments(
                subscription_id=subscription_id, principal_id=principal_id, scope=scope
            )
    ...
    async def get_rbac_recommendations(self, subscription_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            owner_assignments = [
                a for a in assignments if "Owner" in a.get("role_definition_name", "")
            ]
            ...
            logger.error(
                "get_rbac_recommendations_failed", error=str(e), subscription_id=subscription_id
            )
            return []
