...
class CostManagementService:
    ...
    async def _get_cost_client(self, subscription_id: str) -> CostManagementClient:
        ...
        if subscription_id not in self.cost_clients:
            ...
            self.cost_clients[subscription_id] = CostManagementClient(credential, subscription_id)
    ...
    async def get_usage_details(
        self, subscription_id: str, start_date: str, end_date: str, granularity: str = "Daily"
    ) -> CostResponse:
        ...
        try:
            ...
            logger.error("get_usage_details_failed", error=str(e), subscription_id=subscription_id)
            raise Exception(f"Failed to get usage details: {str(e)}")

    async def get_cost_forecast(
        self, subscription_id: str, forecast_days: int = 30
    ) -> CostResponse:
        ...
        try:
            ...
            query = {
                ...
                "timeframe": "Custom",
                "time_period": {"from": start_date.isoformat(), "to": end_date.isoformat()},
                "dataset": {
                    "granularity": "Daily",
            ...
            if result.rows:
                for row in result.rows:
                    ...
                    forecast_breakdown.append(
                        {"date": date_str, "forecast_cost": cost, "currency": "USD"}
                    )
            ...
            logger.error("get_cost_forecast_failed", error=str(e), subscription_id=subscription_id)
            raise Exception(f"Failed to get cost forecast: {str(e)}")

    async def list_budgets(self, subscription_id: str) -> List[Dict[str, Any]]:
                ...
                budgets.append(
                    {
                        ...
                        "forecast_spend": (
                            budget.forecast_spend.amount if budget.forecast_spend else None
                        ),
                    }
                )

            logger.info("budgets_listed", subscription_id=subscription_id, count=len(budgets))

            return budgets

        except AzureError as e:
            logger.error("list_budgets_failed", error=str(e), subscription_id=subscription_id)
            raise Exception(f"Failed to list budgets: {str(e)}")

    async def create_budget(
        self, subscription_id: str, budget_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Create a new budget."""
        try:
            consumption_client = await self._get_consumption_client(subscription_id)
            ...
            budget = await consumption_client.budgets.create_or_update(
                budget_name=budget_data["name"], parameters=budget_params
            )
    ...
    async def get_cost_recommendations(self, subscription_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if usage_data.cost_breakdown:
                ...
                sorted_costs = sorted(
                    usage_data.cost_breakdown, key=lambda x: x.get("cost", 0), reverse=True
                )
                        ...
                        recommendations.append(
                            {
                                ...
                                "potential_savings": service.get("cost", 0)
                                * 0.2,  # Assume 20% savings
                                "actions": [
            ...
            logger.error(
                "get_cost_recommendations_failed", error=str(e), subscription_id=subscription_id
            )
            return []
        ...
        try:
            ...
            return {
                ...
                "budgets_over_threshold": len(
                    [b for b in budgets if b.get("percentage_used", 0) > 80]
                ),
            }

        except Exception as e:
            logger.error("get_budget_status_failed", error=str(e), subscription_id=subscription_id)
            return {"has_budgets": False, "error": str(e)}
