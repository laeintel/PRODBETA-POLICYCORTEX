...
def mock_policy_service():
    ...
    with patch(
        "backend.services.azure_integration.services.PolicyManagementService"
    ) as mock_service:
        mock_instance = MagicMock()
        mock_service.return_value = mock_instance

        # Mock policy operations
        mock_instance.list_policies = AsyncMock(
            return_value=[
                {
                    "id": "policy-1",
                    "name": "Test Policy 1",
        ...
        mock_instance.get_policy = AsyncMock(
            return_value={
                "id": "policy-1",
                "name": "Test Policy 1",
                "type": "custom",
        ...
        mock_instance.create_policy = AsyncMock(
            return_value={"id": "new-policy-id", "name": "New Policy", "status": "created"}
        )
        mock_instance.update_policy = AsyncMock(
            return_value={"id": "policy-1", "name": "Updated Policy", "status": "updated"}
        )
        mock_instance.delete_policy = AsyncMock(return_value=True)
        mock_instance.get_policy_compliance = AsyncMock(
...
def mock_rbac_service():
    ...
    with patch("backend.services.azure_integration.services.RBACManagementService") as mock_service:
        ...
        mock_instance.list_roles = AsyncMock(
            return_value=[{"id": "role-1", "name": "Contributor", "type": "BuiltInRole"}]
        )
        mock_instance.list_role_assignments = AsyncMock(
            return_value=[
                {
                    "id": "assignment-1",
                    "principalId": "user-id",
        ...
        mock_instance.create_role_assignment = AsyncMock(
            return_value={"id": "new-assignment-id", "status": "created"}
        )
...
def mock_cost_service():
    ...
    with patch("backend.services.azure_integration.services.CostManagementService") as mock_service:
        ...
        mock_instance.get_usage_details = AsyncMock(
            return_value={
                "costs": [
        ...
        mock_instance.get_cost_forecast = AsyncMock(
            return_value={
                "forecast": [
        ...
        mock_instance.list_budgets = AsyncMock(
            return_value=[
                {"id": "budget-1", "name": "Test Budget", "amount": 1000.00, "spent": 500.00}
            ]
        )
        mock_instance.get_cost_recommendations = AsyncMock(
            return_value=[{"type": "shutdown", "resource": "vm-1", "potential_savings": 200.00}]
        )
...
def mock_network_service():
    ...
    with patch(
        "backend.services.azure_integration.services.NetworkManagementService"
    ) as mock_service:
        mock_instance = MagicMock()
        mock_service.return_value = mock_instance

        # Mock network operations
        mock_instance.list_virtual_networks = AsyncMock(
            return_value=[
                {
                    "id": "vnet-1",
                    "name": "test-vnet",
        ...
        mock_instance.list_network_security_groups = AsyncMock(
            return_value=[
                {"id": "nsg-1", "name": "test-nsg", "location": "eastus", "securityRules": []}
            ]
        )
        mock_instance.analyze_network_security = AsyncMock(
            return_value={"vulnerabilities": [], "recommendations": [], "score": 85}
        )
...
def mock_resource_service():
    ...
    with patch(
        "backend.services.azure_integration.services.ResourceManagementService"
    ) as mock_service:
        mock_instance = MagicMock()
        mock_service.return_value = mock_instance
        ...
        mock_instance.get_resource = AsyncMock(
            return_value={
                "id": "resource-1",
                "name": "test-vm",
                "type": "Microsoft.Compute/virtualMachines",
        ...
        mock_instance.list_resource_groups = AsyncMock(
            return_value=[{"id": "rg-1", "name": "test-rg", "location": "eastus"}]
        )
...
def policy_request():
    ...
    return {
        ...
        "type": "custom",
        "rules": [
            {"field": "type", "operator": "equals", "value": "Microsoft.Compute/virtualMachines"}
        ],
        "effects": ["deny"],
    }
...
def mock_prometheus_metrics():
    ...
    with patch("backend.services.azure_integration.main.REQUEST_COUNT") as mock_counter, patch(
        "backend.services.azure_integration.main.REQUEST_DURATION"
    ) as mock_histogram, patch(
        "backend.services.azure_integration.main.AZURE_API_CALLS"
