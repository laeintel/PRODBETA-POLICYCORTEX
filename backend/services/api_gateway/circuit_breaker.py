...
class CircuitBreaker:
    ...
    def _log_state_change(self, from_state: CircuitBreakerState, to_state: CircuitBreakerState) -> None:
...
class AdvancedCircuitBreaker(CircuitBreaker):
    ...
    def can_execute(self) -> bool:
        ...
        if self.state == CircuitBreakerState.CLOSED:
            ...
            if self.last_failure_time and (current_time - self.last_failure_time) >= self.current_timeout:
                self._transition_to_half_open()
                return True
            return False
