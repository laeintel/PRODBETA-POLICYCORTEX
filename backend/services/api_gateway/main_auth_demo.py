...
class MockAuthManager:
    ...
    def authenticate_user(self, email: str, **kwargs) -> tuple:
        ...
        refresh_token = f"demo_refresh_{hashlib.sha256(f'refresh_{token_data}'.encode()).hexdigest()[:32]}"
...
async def refresh_token(request: RefreshTokenRequest):
    ...
    new_token = f"demo_token_refreshed_{hashlib.sha256(str(current_time).encode()).hexdigest()[:32]}"
...
async def debug_tokens():
    ...
    return {
        "active_tokens": len(active_tokens),
        "tokens": [{"token": token[:20] + "...", "user": user["email"]} for token, user in active_tokens.items()]
    }
