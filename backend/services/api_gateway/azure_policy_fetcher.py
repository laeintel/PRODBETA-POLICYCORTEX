...
def get_all_subscriptions() -> List[Dict[str, str]]:
    ...
    try:
        ...
        return [{"id": sub["id"], "name": sub["name"]} for sub in subscriptions if sub["state"] == "Enabled"]
...
def get_policy_compliance_state(assignment_id: str, subscription_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if os.name == 'nt':
            ...
            result = subprocess.run(
                ["cmd.exe", "/c", "az", "policy", "state", "summarize", "--policy-assignment", assignment_id, "--output", "json"],
                capture_output=True,
                text=True,
                check=True
            )
            ...
            result = subprocess.run(
                ["az", "policy", "state", "summarize", "--policy-assignment", assignment_id, "--output", "json"],
                capture_output=True,
                text=True,
                check=True
            )
        ...
        try:
            ...
            for assignment in assignments:
                ...
                processed_assignment = {
                    ...
                    "type": "Initiative" if "policySetDefinitions" in assignment.get("policyDefinitionId", "") else "Policy",
...
if __name__ == "__main__":
    ...
    for assignment in assignments:
        print(f"- {assignment['displayName']}: {assignment['complianceState']} ({assignment['resourceCompliance']})")
