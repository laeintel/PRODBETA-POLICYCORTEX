...
from shared.database import async_db_transaction, AuditLog, DatabaseUtils
...
class ComprehensiveAuditLogger:
    ...
    def _initialize_azure_monitor(self):
        ...
        try:
            ...
            self.event_latency = meter.create_histogram(
                "audit_event_latency",
                description="Latency of audit event processing",
                unit="ms"
            )
        ...
        try:
            if not self.logs_client:
                ...
                self.logs_client = LogsIngestionClient(
                    endpoint=settings.azure.log_analytics_endpoint,
    ...
    async def _trigger_security_alert(self, audit_event: Dict[str, Any]) -> None:
        ...
        alert = {
            "alert_id": hashlib.sha256(f"alert_{audit_event['event_id']}".encode()).hexdigest()[:16],
            "timestamp": datetime.utcnow().isoformat(),
    ...
    async def _batch_processor(self) -> None:
        ...
        while True:
            try:
                ...
                if len(self._audit_queue) >= self._batch_size or \
    ...
    async def query_audit_logs(
        ...
        with tracer.start_as_current_span("query_audit_logs") as span:
            ...
            while current_date <= end_date:
                ...
                events_json = await redis_client.zrangebyscore(
                    audit_key,
                    start_score,
                    end_score
                )
    ...
    async def generate_compliance_report(
        ...
        report["compliance_score"] = (actual_events / expected_events * 100) if expected_events > 0 else 0
        ...
        if report["compliance_score"] < 100:
            report["recommendations"].append(
                "Ensure all required event types are being logged"
            )
