...
class DataTransformer:
    ...
    async def transform_data(self,
                           data: pd.DataFrame,
                           transformations: List[TransformationRule]) -> pd.DataFrame:
        """
                ...
                if column_name and formula:
                    try:
                        ...
                        if formula == 'current_timestamp':
                            ...
                            parts = formula.split('+')
                            if len(parts) == 2 and all(p.strip() in enriched_data.columns for p in parts):
                                enriched_data[column_name] = enriched_data[parts[0].strip()] + enriched_data[parts[1].strip()]
                        elif '*' in formula:
                            parts = formula.split('*')
                            if len(parts) == 2 and all(p.strip() in enriched_data.columns for p in parts):
                                enriched_data[column_name] = enriched_data[parts[0].strip()] * enriched_data[parts[1].strip()]
    ...
    def create_transformation_rule(self,
                                 rule_id: str,
                                 name: str,
                                 transformation_type: TransformationType,
                                 config: Dict[str, Any],
                                 priority: int = 0) -> TransformationRule:
    ...
    async def detect_data_types(self, data: pd.DataFrame) -> Dict[str, str]:
        ...
        for column in data.columns:
            ...
            if current_type == 'object':
                ...
                if len(unique_values) <= 2 and all(v in ['true', 'false', '1', '0', 'yes', 'no'] for v in unique_values if pd.notna(v)):
