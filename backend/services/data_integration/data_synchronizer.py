...
class DataSynchronizer:
    ...
    async def sync_data(self, rule_id: str) -> SyncResult:
        ...
        result = SyncResult(
            rule_id=rule_id, status=SyncStatus.RUNNING, start_time=datetime.utcnow()
        )
    ...
    async def _sync_full(self, rule: SyncRule, result: SyncResult) -> SyncResult:
        ...
        source_data = await self.data_connector.read_data(rule.source, filters=rule.filters)
        ...
        success = await self.data_connector.write_data(rule.target, source_data, mode="replace")
    ...
    async def _sync_incremental(self, rule: SyncRule, result: SyncResult) -> SyncResult:
        ...
        source_data = await self.data_connector.read_data(rule.source, filters=incremental_filter)
        ...
        # Write to target
        success = await self.data_connector.write_data(rule.target, source_data, mode=mode)
    ...
    async def _sync_cdc(self, rule: SyncRule, result: SyncResult) -> SyncResult:
        ...
        source_data = await self.data_connector.read_data(rule.source, filters=cdc_filter)
        ...
        if not creates.empty:
            success = await self.data_connector.write_data(
                rule.target, creates.drop(["_operation"], axis=1, errors="ignore"), mode="append"
            )
            if success:
        ...
        if not updates.empty:
            success = await self.data_connector.write_data(
                rule.target, updates.drop(["_operation"], axis=1, errors="ignore"), mode="upsert"
            )
            if success:
    ...
    async def _sync_snapshot(self, rule: SyncRule, result: SyncResult) -> SyncResult:
        ...
        if not target_data.empty and primary_key:
            ...
            new_records = new_records[
                [
                    ...
                    if not col.endswith("_target") and col != "_merge"
                ]
            ]
            ...
            for _, row in both_records.iterrows():
                ...
                for s_col, t_col in zip(source_cols, target_cols):
                    if pd.isna(row[s_col]) != pd.isna(row[t_col]) or (
                        not pd.isna(row[s_col]) and row[s_col] != row[t_col]
                    ):
                        has_changes = True
                        break
        ...
        if not new_records.empty:
            success = await self.data_connector.write_data(rule.target, new_records, mode="append")
            if success:
                result.records_created = len(new_records)

        if not changed_records.empty:
            success = await self.data_connector.write_data(
                rule.target, changed_records, mode="upsert"
            )
            if success:
    ...
    def get_sync_status(self, rule_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if rule_id:
            ...
            rule = self.sync_rules[rule_id]
            recent_results = [r for r in self.sync_history if r.rule_id == rule_id][
                -5:
            ]  # Last 5 runs
