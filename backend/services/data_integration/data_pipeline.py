...
class DataPipeline:
    ...
    async def _execute_pipeline_steps(self, pipeline: Pipeline, run: PipelineRun):
        ...
        while len(executed_steps) < len(pipeline.steps):
            ...
            if not ready_steps:
                ...
                remaining_steps = [
                    s for s in pipeline.steps if s.id not in executed_steps and s.enabled
                ]
                if remaining_steps:
                    raise Exception(
                        f"Circular dependency detected or unresolvable dependencies in steps: {[s.id for s in remaining_steps]}"
                    )
                break
            ...
            for step in ready_steps:
                ...
                try:
                    ...
                    if step.retry_count < step.max_retries:
                        step.retry_count += 1
                        logger.info(
                            f"Retrying step {step.id} (attempt {step.retry_count}/{step.max_retries})"
                        )
    ...
    async def _execute_step(
        self, step: PipelineStep, step_data: Dict[str, Any], run: PipelineRun
    ) -> Any:
        ...
        try:
            if step.type == StepType.EXTRACT:
                ...
                return await asyncio.wait_for(
                    self._execute_transform_step(step, step_data), timeout
                )
    ...
    async def _execute_extract_step(
        self, step: PipelineStep, step_data: Dict[str, Any]
    ) -> pd.DataFrame:
    ...
    async def _execute_transform_step(
        ...
        if not input_step or input_step not in step_data:
            raise ValueError(
                f"Transform step requires valid 'input_step' in config. Available: {list(step_data.keys())}"
            )
    ...
    async def _execute_load_step(self, step: PipelineStep, step_data: Dict[str, Any]) -> bool:
        ...
        if not input_step or input_step not in step_data:
            raise ValueError(
                f"Load step requires valid 'input_step' in config. Available: {list(step_data.keys())}"
            )
    ...
    async def _execute_validate_step(
        ...
        if not input_step or input_step not in step_data:
            raise ValueError(
                f"Validate step requires valid 'input_step' in config. Available: {list(step_data.keys())}"
            )
    ...
    async def _execute_custom_step(self, step: PipelineStep, step_data: Dict[str, Any]) -> Any:
        ...
        if not function_name or function_name not in self.custom_functions:
            raise ValueError(
                f"Custom step requires valid 'function' in config. Available: {list(self.custom_functions.keys())}"
            )
    ...
    def _create_validation_template(self, **params) -> Pipeline:
        ...
        steps = [
            PipelineStep(
                ...
                type=StepType.EXTRACT,
                config={"source": params.get("source"), "query": params.get("query")},
            ),
            PipelineStep(
                id="validate",
    ...
    def _create_sync_template(self, **params) -> Pipeline:
        ...
        steps = [
            PipelineStep(
                ...
                type=StepType.SYNC,
                config={"sync_rule_id": params.get("sync_rule_id")},
            )
        ]
