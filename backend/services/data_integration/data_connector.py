...
class DataConnector:
    ...
    async def write_data(self,
        ...
        try:
            if source_type == DataSourceType.POSTGRESQL:
                ...
                return await self._write_mongodb(connection, conn_info['config'], data, table_name, mode)
    ...
    async def _read_postgresql(self,
    ...
    async def _write_postgresql(self,
                               connection: asyncpg.Connection,
                               data: pd.DataFrame,
                               table_name: str,
                               mode: str) -> bool:
    ...
    async def _write_blob(self,
        ...
        blob_name = f"{config.get('prefix', 'data')}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.parquet"
    ...
    async def _stream_event_hub(self,
    ...
    async def _stream_service_bus(self,
