...
async def provision_organization(
    ...
    try:
        ...
        background_tasks.add_task(
            provision_organization_task, tenant_id, organization_name, template, configuration
        )
...
async def provision_organization_task(
    tenant_id: str, organization_name: str, template: str, configuration: Dict[str, Any]
):
    """Background task for organization provisioning"""
    try:
        resources = await organization_provisioner.provision_organization(
            tenant_id, organization_name, template, configuration
        )
...
async def create_subscription(tenant_id: str, request: CreateSubscriptionRequest):
    ...
    try:
        subscription = await subscription_manager.create_subscription(
            tenant_id, request.plan_id, request.billing_cycle
        )
...
async def upgrade_subscription(subscription_id: str, new_plan_id: str, immediate: bool = True):
    """Upgrade a subscription"""
    success = await subscription_manager.upgrade_subscription(
        subscription_id, new_plan_id, immediate
    )
...
async def add_payment_method(tenant_id: str, request: PaymentMethodRequest):
    ...
    try:
        method = await billing_integration.add_payment_method(
            tenant_id, request.provider, request.method_type, request.details
        )
...
async def handle_billing_webhook(provider: str, payload: Dict[str, Any], signature: str):
    ...
    try:
        ...
        result = await billing_integration.process_webhook(provider_enum, payload, signature)
...
async def record_usage(
    tenant_id: str, metric: str, value: float, metadata: Optional[Dict[str, Any]] = None
):
