...
class SubscriptionManager:
    ...
    async def create_subscription(self,
        ...
        logger.info(f"Created subscription {subscription_id} for tenant {tenant_id} on plan {plan_id}")
    ...
    async def activate_subscription(self, subscription_id: str) -> bool:
        ...
        if subscription.status != SubscriptionStatus.PENDING:
            logger.warning(f"Cannot activate subscription {subscription_id} with status {subscription.status}")
            return False
    ...
    async def upgrade_subscription(self,
        ...
        if immediate:
            ...
            old_plan_price = self.plans[current_plan_id].pricing.get(subscription.billing_cycle.value, 0)
            new_plan_price = self.plans[new_plan_id].pricing.get(subscription.billing_cycle.value, 0)
        ...
        logger.info(f"Upgraded subscription {subscription_id} from {current_plan_id} to {new_plan_id}")
        
        return True
        
    async def downgrade_subscription(self,
                                   subscription_id: str,
                                   new_plan_id: str) -> bool:
        """
    ...
    async def reactivate_subscription(self, subscription_id: str) -> bool:
        ...
        subscription.status = SubscriptionStatus.ACTIVE
        subscription.metadata['reactivation'] = {
    ...
    def get_usage_summary(self, tenant_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        for record in tenant_usage:
            ...
            usage_by_metric[record.metric]['min'] = min(usage_by_metric[record.metric]['min'], record.value)
            usage_by_metric[record.metric]['max'] = max(usage_by_metric[record.metric]['max'], record.value)
