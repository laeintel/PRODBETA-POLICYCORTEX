...
class BillingIntegration:
    ...
    async def _process_stripe_payment(
        self, payment: PaymentDetails, payment_method_id: Optional[str]
    ) -> Dict[str, Any]:
    ...
    async def _process_invoice_payment(self, payment: PaymentDetails) -> Dict[str, Any]:
        ...
        invoice = await self.create_invoice(
            payment.tenant_id, payment.amount, payment.currency, due_days=30
        )

        payment.metadata["invoice_id"] = invoice.invoice_id

        return {"success": True, "reference_id": invoice.invoice_number, "error": None}
    ...
    async def remove_payment_method(self, tenant_id: str, method_id: str) -> bool:
        """
        ...
        if method.tenant_id != tenant_id:
            logger.warning(
                f"Tenant {tenant_id} attempted to remove method owned by {method.tenant_id}"
            )
            return False
    ...
    async def set_default_payment_method(self, tenant_id: str, method_id: str) -> bool:
        """
    ...
    def get_payment_history(self, tenant_id: str, limit: int = 100) -> List[Dict[str, Any]]:
        """Get payment history for a tenant"""

        tenant_payments = [p for p in self.payments.values() if p.tenant_id == tenant_id]
    ...
    def get_payment_methods(self, tenant_id: str) -> List[Dict[str, Any]]:
        ...
        methods = [m for m in self.payment_methods.values() if m.tenant_id == tenant_id]
    ...
    def get_invoices(self, tenant_id: str) -> List[Dict[str, Any]]:
        ...
        tenant_invoices = [i for i in self.invoices.values() if i.tenant_id == tenant_id]
