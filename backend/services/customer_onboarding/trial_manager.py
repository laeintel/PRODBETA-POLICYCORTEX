...
class TrialManager:
            ...
            'email_campaigns': [
                {
                    'day': 1,
                    'template': 'welcome',
                    'subject': 'Welcome to PolicyCortex!'
                },
                {
                    'day': 3,
    ...
    async def start_trial(self,
        ...
        await self.record_activity(
            tenant_id,
            'trial_started',
            {'plan_id': plan_id, 'duration_days': trial_days}
        )
    ...
    def _get_trial_features(self, plan_id: str) -> List[TrialFeature]:
        ...
        if plan_id in ['professional', 'enterprise']:
            return [
                TrialFeature.FULL_ACCESS,
                TrialFeature.SUPPORT,
                TrialFeature.TRAINING
            ]
        else:
            return [
                TrialFeature.LIMITED_ACCESS,
                TrialFeature.DEMO_DATA,
                TrialFeature.SUPPORT
            ]
    ...
    async def extend_trial(self,
        ...
        await self.record_activity(
            trial.tenant_id,
            'trial_extended',
            {'days': additional_days, 'reason': reason}
        )
    ...
    async def _check_conversion_triggers(self,
                                       tenant_id: str,
                                       activity: TrialActivity) -> None:
        ...
        if activity.activity_type == 'feature_usage':
            usage_count = sum(
                1 for a in self.trial_activities
    ...
    async def _trigger_engagement_campaign(self,
                                         trial: Trial,
                                         trigger: str) -> None:
        """Trigger engagement campaign"""
    ...
    async def _trigger_upgrade_campaign(self,
                                      trial: Trial,
                                      trigger: str) -> None:
        """Trigger upgrade campaign"""
    ...
    def _calculate_conversion_score(self, trial: Trial) -> float:
        ...
        total_activities = sum(
            1 for a in self.trial_activities
            if a.tenant_id == trial.tenant_id
        )
    ...
    def get_conversion_analytics(self) -> Dict[str, Any]:
        ...
        avg_activities = sum(activities_per_trial.values()) / len(activities_per_trial) if activities_per_trial else 0
    ...
    async def get_expiring_trials(self, days_ahead: int = 3) -> List[Trial]:
        ...
        expiring = []
        for trial in self.trials.values():
            if (trial.status == TrialStatus.ACTIVE and
                trial.expires_at <= expiry_threshold):
