...
class AzureConnectorService:
    ...
    async def _get_blob_service_client(self) -> BlobServiceClient:
        ...
        if self.blob_service_client is None:
            ...
            account_url = f"https://{self.settings.azure.storage_account_name}.blob.core.windows.net"
    ...
    async def _get_eventhub_client(self, eventhub_name: str, connection_string: str) -> EventHubProducerClient:
    ...
    async def _get_table_service_client(self) -> TableServiceClient:
        ...
        if self.table_service_client is None:
            credential = await self._get_credential()
            account_url = f"https://{self.settings.azure.storage_account_name}.table.core.windows.net"
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            unhealthy_connectors = [c for c in health_status["connectors"] if c["status"] != "healthy"]
            if unhealthy_connectors:
                health_status["status"] = "degraded" if len(unhealthy_connectors) < len(health_status["connectors"]) else "unhealthy"
