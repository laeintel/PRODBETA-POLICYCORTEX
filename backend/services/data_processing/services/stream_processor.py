...
class StreamProcessorService:
    ...
    async def _process_event_hub_stream(self, processor_id: str, processor_config: Dict[str, Any]) -> None:
        """Process Event Hub stream."""
        try:
            source_config = DataSourceConfig(**processor_config["source_config"])
            output_config = DataTargetConfig(**processor_config["output_config"])
            processing_rules = [TransformationRule(**rule) for rule in processor_config["processing_rules"]]
    ...
    async def _process_service_bus_stream(self, processor_id: str, processor_config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            connection_string = source_config.connection_string or self.settings.azure.service_bus_connection_string
            ...
            async with ServiceBusClient.from_connection_string(connection_string) as client:
                ...
                if queue_name:
                    ...
                    raise ValueError("Either queue_name or (topic_name and subscription_name) must be provided")
    ...
    async def _process_message_batch(self, processor_id: str, messages: List[Dict[str, Any]], 
        ...
        try:
            ...
            metrics["processing_rate"] = len(messages) / processing_time if processing_time > 0 else 0
    ...
    async def _send_to_event_hub(self, messages: List[Dict[str, Any]], output_config: DataTargetConfig) -> None:
    ...
    async def _send_to_service_bus(self, messages: List[Dict[str, Any]], output_config: DataTargetConfig) -> None:
        """Send messages to Service Bus."""
        try:
            connection_string = output_config.connection_string or self.settings.azure.service_bus_connection_string
    ...
    async def _send_to_cosmos_db(self, messages: List[Dict[str, Any]], output_config: DataTargetConfig) -> None:
