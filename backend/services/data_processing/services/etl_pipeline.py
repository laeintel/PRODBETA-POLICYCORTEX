...
class ETLPipelineService:
        ...
        if self.spark_session is None or self.spark_session.sparkContext._jsc is None:
            try:
                ...
                builder = SparkSession.builder \
                    ...
                    .config("spark.sql.adaptive.coalescePartitions.enabled", str(spark_config.sql_adaptive_coalesce_partitions).lower())
    ...
    async def execute_pipeline(self, pipeline_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def _execute_pandas_pipeline(self, pipeline_config: Dict[str, Any], 
        ...
        try:
            ...
            transformation_rules = [TransformationRule(**rule) for rule in pipeline_config["transformation_rules"]]
            validation_rules = [ValidationRule(**rule) for rule in pipeline_config["validation_rules"]]
            ...
            if validation_rules:
                ...
                execution_record["details"]["validation_results"] = validation_result["validation_results"]
    ...
    async def _read_data_with_spark(self, spark: SparkSession, source_config: DataSourceConfig):
        ...
        try:
            ...
            if source_type == "azure_sql":
                ...
                df = spark.read \
                    .format("jdbc") \
                    .option("url", f"jdbc:sqlserver://{source_config.server}:1433;databaseName={source_config.database}") \
    ...
    async def _apply_spark_transformations(self, spark: SparkSession, df, transformation_rules: List[TransformationRule]):
        """Apply transformations using Spark."""
        try:
    ...
    async def _apply_spark_validation(self, spark: SparkSession, df, validation_rules: List[ValidationRule]) -> float:
            ...
            for rule in validation_rules:
                if rule.rule_type == "not_null":
                    ...
                    if min_val is not None and max_val is not None:
                        invalid_count = df.filter(~((df[field] >= min_val) & (df[field] <= max_val))).count()
    ...
    async def _write_data_with_spark(self, spark: SparkSession, df, target_config: DataTargetConfig) -> int:
        ...
        try:
            ...
            if target_type == "azure_sql":
                ...
                df.write \
                    .format("jdbc") \
                    .option("url", f"jdbc:sqlserver://{target_config.server}:1433;databaseName={target_config.database}") \
                ...
                blob_name = target_config.additional_config.get("blob_name", f"data_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}")
    ...
    async def _update_pipeline_metrics(self, pipeline_id: str, execution_record: Dict[str, Any], success: bool) -> None:
        ...
        try:
            ...
            if success:
                ...
                metrics["total_processing_time"] += execution_record.get("processing_time_seconds", 0)
                ...
                metrics["average_quality_score"] = (current_avg * (metrics["success_count"] - 1) + new_score) / metrics["success_count"]
            else:
