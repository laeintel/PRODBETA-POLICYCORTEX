...
class AuthManager:
    ...
    async def audit_data_access(self, user_info: Dict[str, Any], operation: str, 
        ...
        try:
            ...
            redis_client = await self._get_redis_client()
            audit_key = f"audit:{datetime.utcnow().strftime('%Y-%m-%d')}:{user_info.get('id')}:{uuid.uuid4()}"
        ...
        try:
            ...
            if "export" in request_data.get("operation", ""):
                ...
                if export_restrictions.get("max_records"):
                    validation_result["applied_restrictions"]["max_records"] = export_restrictions["max_records"]
            ...
            if "subscription_restrictions" in scope:
                ...
                if subscription_restrictions and request_data.get("subscription_id") not in subscription_restrictions:
                    validation_result["is_valid"] = False
