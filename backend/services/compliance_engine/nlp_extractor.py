...
class NLPPolicyExtractor:
    ...
    async def extract_policies(
        self, text: str, document_context: Optional[Dict[str, Any]] = None
        ...
        if self.text_analytics_client:
            validated_policies = await self._enhance_with_text_analytics(validated_policies, text)
    ...
    def _extract_with_spacy(self, text: str) -> List[ExtractedPolicy]:
        ...
        for sent in doc.sents:
            ...
            has_requirement = any(
                token.text.lower() in ["must", "shall", "should", "required"] for token in sent
            )
    ...
    def _merge_policies(
        self, openai_policies: List[ExtractedPolicy], pattern_policies: List[ExtractedPolicy]
    ) -> List[ExtractedPolicy]:
                ...
                for existing in unique_policies:
                    if existing.description[:100].lower() == description_key:
                        ...
                        existing.confidence_score = (
                            existing.confidence_score + policy.confidence_score
                        ) / 2
                        break
    ...
    async def _enhance_with_text_analytics(
        self, policies: List[ExtractedPolicy], full_text: str
    ) -> List[ExtractedPolicy]:
        ...
        try:
            ...
            for policy in policies:
                policy.metadata["sentiment"] = (
                    sentiment_result[0].sentiment if sentiment_result else None
                )
                policy.metadata["key_phrases"] = (
                    key_phrases_result[0].key_phrases[:10] if key_phrases_result else []
                )
                ...
                if entities_result:
                    relevant_entities = [
                        e.text
                        for e in entities_result[0].entities
    ...
    async def extract_compliance_rules(
    ...
    def _generate_evaluation_criteria(self, policy: ExtractedPolicy) -> Dict[str, Any]:
        ...
        criteria = {"type": "composite", "operator": "AND", "conditions": []}
        ...
        for condition in policy.conditions:
            criteria["conditions"].append({"type": "condition", "details": condition})
