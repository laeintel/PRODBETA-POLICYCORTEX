...
class RuleEngine:
    ...
    async def evaluate_rules(
        ...
        for rule in rules_to_evaluate:
            ...
            try:
                result = await self._evaluate_single_rule(rule, resource, execute_actions)
    ...
    async def _evaluate_single_rule(
        self, rule: ComplianceRule, resource: Dict[str, Any], execute_actions: bool
    ) -> RuleExecutionResult:
    ...
    async def _execute_action(
        self, action: Dict[str, Any], resource: Dict[str, Any], rule: ComplianceRule
    ) -> Dict[str, Any]:
    ...
    async def _action_alert(
        ...
        message = action.get(
            "message", f"Rule {rule.name} matched for resource {resource.get('id')}"
        )
    ...
    async def _action_remediate(
        ...
        if rule.remediation_script:
            ...
            return {"success": True, "remediation_type": "script", "script_executed": True}
            ...
            return {"success": True, "remediation_type": "steps", "steps_executed": steps_executed}
    ...
    async def _action_notify(
        ...
        logger.info(f"Sending notifications via {channels} to {recipients}", rule_id=rule.rule_id)

        return {"success": True, "channels": channels, "recipients_count": len(recipients)}
    ...
    async def _action_block(
        ...
        return {"success": True, "block_type": block_type, "duration": duration}

    async def _action_custom(
        self, action: Dict[str, Any], resource: Dict[str, Any], rule: ComplianceRule
    ) -> Dict[str, Any]:
    ...
    def export_rules(self, format: str = "json") -> str:
        ...
        rules_data = {rule_id: rule.dict() for rule_id, rule in self.rules.items()}
