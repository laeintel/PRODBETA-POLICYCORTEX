...
class DocumentProcessor:
    ...
    async def upload_document(self,
        ...
        document_id = f"{tenant_id}/{datetime.utcnow().strftime('%Y%m%d')}/{content_hash[:8]}_{filename}"
    ...
    async def _process_document_async(self, 
    ...
    async def get_document(self, document_id: str) -> ProcessedDocument:
        ...
        return ProcessedDocument(
            ...
            processed_at=datetime.fromisoformat(properties.metadata.get('processed_at')) 
                        if properties.metadata.get('processed_at') else None
        )
