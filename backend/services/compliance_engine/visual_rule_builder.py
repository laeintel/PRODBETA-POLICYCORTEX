...
class VisualRuleBuilder:
    ...
    def _load_default_templates(self) -> Dict[str, RuleTemplate]:
        ...
        templates["encryption_required"] = RuleTemplate(
            ...
            base_rule={
                ...
                "conditions": [{"type": "custom", "function": "check_encryption"}],
                "actions": [{"type": RuleAction.ALERT, "level": "high"}],
                "severity": "high",
            },
        ...
        templates["public_access_blocked"] = RuleTemplate(
            ...
            base_rule={
                "rule_type": RuleType.SECURITY,
                "conditions": [{"type": "custom", "function": "check_public_access"}],
                "actions": [{"type": RuleAction.BLOCK, "block_type": "access"}],
                "severity": "critical",
            },
                ...
                "conditions": [
                    {
                        "type": "custom",
                        "function": "check_tags",
                        "parameters": {"required_tags": ["Environment", "Owner", "CostCenter"]},
                    }
                ],
                "actions": [{"type": RuleAction.TAG, "tags": {"ComplianceStatus": "NonCompliant"}}],
                "severity": "medium",
            },
        ...
        templates["cost_threshold"] = RuleTemplate(
            ...
            base_rule={
                ...
                "actions": [{"type": RuleAction.NOTIFY, "channels": ["email"], "recipients": []}],
                "severity": "medium",
            },
        ...
        templates["backup_required"] = RuleTemplate(
            ...
            base_rule={
                ...
                "conditions": [{"type": "custom", "function": "check_backup"}],
                "actions": [
                    {"type": RuleAction.REMEDIATE, "steps": ["Enable backup configuration"]}
                ],
                "severity": "high",
            },
    ...
    def add_component(self, session_id: str, component: VisualRuleComponent) -> str:
    ...
    def remove_component(self, session_id: str, component_id: str) -> bool:
        ...
        session.components = [c for c in session.components if c.component_id != component_id]
    ...
    def connect_components(
        self, session_id: str, source_id: str, target_id: str, connection_type: str = "flow"
    ) -> bool:
        """Connect two components"""
        session = self.get_session(session_id)

        connection = {"source": source_id, "target": target_id, "type": connection_type}
    ...
    def validate_rule(self, session_id: str) -> Dict[str, Any]:
        ...
        for component in session.components:
            connected = any(
                conn["source"] == component.component_id or conn["target"] == component.component_id
                for conn in session.connections
            )
...
async def connect_components(
    session_id: str, source_id: str, target_id: str, connection_type: str = "flow"
):
    """Connect two components"""
    try:
