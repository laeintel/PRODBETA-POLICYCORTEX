...
class PredictiveAnalytics:
    ...
    async def forecast_with_prophet(self,
                                   data: pd.DataFrame,
                                   periods: int = 30,
        ...
        return {
            ...
            'changepoints': model.changepoints.dt.strftime('%Y-%m-%d').tolist() if len(model.changepoints) > 0 else []
        }
        
    async def predict_capacity_needs(self,
                                   resource_usage: pd.DataFrame,
                                   growth_rate: Optional[float] = None) -> Dict[str, Any]:
        """
