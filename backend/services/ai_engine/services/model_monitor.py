...
class ModelMonitor:
    ...
    async def record_prediction(self, model_name: str, input_data: Dict[str, Any],
        ...
        try:
            ...
            if actual is not None:
                await self._update_performance_metrics(model_name, prediction, actual, inference_time)
    ...
    async def _detect_ks_drift(self, reference_features: np.ndarray,
        ...
        try:
            ...
            for i in range(reference_features.shape[1]):
                ...
                ref_cdf = np.searchsorted(ref_sorted, combined_sorted, side='right') / len(ref_sorted)
                curr_cdf = np.searchsorted(curr_sorted, combined_sorted, side='right') / len(curr_sorted)
    ...
    async def get_model_metrics(self, model_name: str) -> Optional[ModelMetrics]:
        ...
        try:
            ...
            inference_times = [m.get("inference_time", 0) for m in recent_metrics if "inference_time" in m]
    ...
    async def get_model_health(self, model_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(performance_history) > 10:
                ...
                baseline_accuracy = np.mean([m.get("accuracy", 0) for m in performance_history[:10]])
            ...
            if recent_drift:
                ...
                if critical_drift:
                    ...
                    health_status["recommendations"].append("Monitor closely and consider retraining")
