...
class ContextAwareNLU:
    ...
    def _initialize_governance_patterns(self):
        ...
        self.governance_patterns = {
            'policy_reference': re.compile(r'\b(?:policy|procedure|guideline)\s+[\w\-\.]+\b', re.IGNORECASE),
            'resource_id': re.compile(r'\b(?:rg-|vm-|st-|kv-|aks-|app-|db-)\w+\b', re.IGNORECASE),
            'compliance_framework': re.compile(r'\b(?:SOX|GDPR|HIPAA|PCI\s*DSS|ISO\s*27001|NIST)\b', re.IGNORECASE),
            'risk_level': re.compile(r'\b(?:critical|high|medium|low)\s*risk\b', re.IGNORECASE),
            'cost_amount': re.compile(r'\$[\d,]+(?:\.\d{2})?|\b\d+\s*(?:dollars?|USD|cents?)\b', re.IGNORECASE),
            'percentage': re.compile(r'\b\d+(?:\.\d+)?%\b'),
            'time_period': re.compile(r'\b(?:daily|weekly|monthly|quarterly|annually|last\s+\w+|next\s+\w+)\b', re.IGNORECASE)
        }
    ...
    def _calculate_confidence(self, 
        ...
        entity_confidence = np.mean([e.get('confidence', 0.0) for e in entities]) * 0.3 if entities else 0.0
        sentiment_confidence = max([r['score'] for r in sentiment.get('raw_results', [])]) * 0.3 if sentiment.get('raw_results') else 0.0
        ...
        sample_knowledge = [
            {
                ...
                'content': 'All Azure resources must be tagged with Environment, Owner, CostCenter, and Project tags. Resources without proper tags will be flagged for compliance review.',
                'category': 'tagging',
            ...
            },
            {
                ...
                'content': 'Implement auto-scaling, use reserved instances for predictable workloads, and regularly review unutilized resources.',
            ...
            },
            {
                ...
                'content': 'Quarterly audits must verify resource configurations, access controls, data handling procedures, and policy adherence.',
                'category': 'compliance',
    ...
    async def generate_response(self,
        ...
        try:
            ...
            if self._should_add_greeting(conversation_history, context):
                greeting = self.response_templates.get(context, {}).get('greeting', 'How can I help you today?')
...
class ConversationGovernanceIntelligence:
    ...
    async def get_conversation_insights(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_common_context': max(context_distribution.items(), key=lambda x: x[1])[0] if context_distribution else None,
            'insights': [
                f"User has {total_turns} conversation turns across {len(user_sessions)} sessions",
                f"Most common topic: {max(context_distribution.items(), key=lambda x: x[1])[0] if context_distribution else 'N/A'}",
