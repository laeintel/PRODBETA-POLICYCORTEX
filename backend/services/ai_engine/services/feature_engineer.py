...
class FeatureEngineer:
        ...
        try:
            ...
            for key, value in data.items():
                if isinstance(value, str) and len(value) > 0:
                    ...
                    features[f"{key}_uppercase_ratio"] = sum(c.isupper() for c in value) / len(value)
                    features[f"{key}_digit_ratio"] = sum(c.isdigit() for c in value) / len(value)
                    features[f"{key}_special_char_ratio"] = sum(not c.isalnum() and not c.isspace() for c in value) / len(value)
                    ...
                    features[f"{key}_positive_words"] = sum(word in value_lower for word in positive_words)
                    features[f"{key}_negative_words"] = sum(word in value_lower for word in negative_words)
    ...
    async def _extract_usage_features(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            usage_fields = []
            for key, value in data.items():
                if any(term in key.lower() for term in ["usage", "utilization", "consumption", "activity"]):
                    if isinstance(value, (int, float)):
