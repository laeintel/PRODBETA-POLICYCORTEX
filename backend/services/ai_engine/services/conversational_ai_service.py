...
class DialogueManager:
    ...
    def _has_sufficient_info(self, context: ConversationContext, intent: IntentType) -> bool:
        ...
        available_info = set(context.extracted_entities.keys()) | set(context.context_variables.keys())
...
class ActionExecutor:
    ...
    async def _handle_policy_analysis(self, context: ConversationContext) -> Dict[str, Any]:
        ...
        try:
            analysis_results = await self.gnn_service.analyze_governance_correlations(governance_data)
    ...
    async def _handle_correlation_analysis(self, context: ConversationContext) -> Dict[str, Any]:
        ...
        try:
            ...
            correlation_results = await self.gnn_service.analyze_governance_correlations(sample_governance_data)
    ...
    async def _handle_general_info(self, context: ConversationContext) -> Dict[str, Any]:
        ...
        return {
            ...
            'message': 'I can help you with Azure governance tasks including resource queries, policy analysis, compliance checks, cost optimization, security assessments, and more.',
            'available_commands': [
...
class ConversationalAIService:
    ...
    def __init__(self):
        ...
        self.response_templates = {
            ConversationState.GREETING: [
                ...
                "Hi there! I can help you with Azure resource management, policies, and compliance. What would you like to know?"
            ],
            ConversationState.INTENT_IDENTIFICATION: [
    ...
    async def _generate_response_text(self, 
        ...
        if next_state == ConversationState.GREETING:
            ...
            if response_data and response_data.get('success'):
                ...
                error = response_data.get('error', 'Unknown error') if response_data else 'Processing failed'
    ...
    def _identify_missing_information(self, context: ConversationContext, intent: IntentType) -> str:
    ...
    def _format_results(self, results: Dict[str, Any]) -> str:
        ...
        result_type = results.get('query_type') or results.get('analysis_type') or results.get('response_type')
        
        if result_type == 'resource_query':
            count = results.get('result_count', 0)
            return f"I found {count} resources matching your criteria. Here's what I discovered: {json.dumps(results.get('results', [])[:3], indent=2)}"
        
        elif result_type == 'policy_analysis':
            return f"I've analyzed your policies. Here are the key findings: {json.dumps(results.get('analysis_results', {}), indent=2)}"
        
        elif result_type == 'compliance_check':
            score = results.get('overall_compliance', 0)
            return f"Your overall compliance score is {score:.0%}. Here are the details: {json.dumps(results.get('domain_scores', {}), indent=2)}"
        
        elif result_type == 'cost_optimization':
            savings = results.get('potential_savings', 0)
            return f"I found potential monthly savings of ${savings:.2f}. Here are the top optimization opportunities: {json.dumps(results.get('optimization_opportunities', [])[:3], indent=2)}"
        
        elif result_type == 'security_assessment':
            score = results.get('overall_score', 0)
            critical = results.get('critical_findings', 0)
            return f"Your security score is {score:.0%} with {critical} critical findings. Here are the key security domains: {json.dumps(results.get('security_domains', {}), indent=2)}"
        
        elif result_type == 'correlation_analysis':
            return f"I've completed the correlation analysis. Here are the key findings: {json.dumps(results.get('key_findings', []), indent=2)}"
        
        elif result_type == 'impact_prediction':
            return f"Based on my analysis, here are the predicted impacts: {json.dumps(results.get('predicted_impacts', []), indent=2)}"
        
        elif result_type == 'help':
            return f"Here are the topics I can help with: {json.dumps(list(results.get('help_topics', {}).keys()), indent=2)}"
        
        return f"Here are your results: {json.dumps(results, indent=2)[:500]}..."
    
    def _get_suggested_actions(self, intent: Optional[IntentType], state: ConversationState) -> List[Dict[str, Any]]:
    ...
    def _create_new_context(self, user_id: str, conversation_id: Optional[str] = None) -> ConversationContext:
    ...
    async def _load_conversation_context(self, conversation_id: str) -> Optional[ConversationContext]:
        ...
        try:
            ...
            if context_data:
                ...
                context_dict['session_start'] = datetime.fromisoformat(context_dict['session_start'])
