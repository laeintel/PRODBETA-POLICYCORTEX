...
class FuzzyRiskAssessment:
    ...
    def setup_fuzzy_system(self):
        ...
        self.violation_probability = ctrl.Antecedent(
            np.arange(0,
            1.1,
            0.1),
        ...
        self.violation_probability['low'] = fuzz.trimf(
            self.violation_probability.universe,
            [0,
            0,
            0.3]
        )
        self.violation_probability['medium'] = fuzz.trimf(
            self.violation_probability.universe,
            [0.2,
            0.5,
            0.8]
        )
        self.violation_probability['high'] = fuzz.trimf(
            self.violation_probability.universe,
            [0.7,
            1.0,
            1.0]
        ...
        self.remediation_complexity['low'] = fuzz.trimf(
            self.remediation_complexity.universe,
            [0,
            0,
            3]
        )
        self.remediation_complexity['medium'] = fuzz.trimf(
            self.remediation_complexity.universe,
            [2,
            5,
            8]
        )
        self.remediation_complexity['high'] = fuzz.trimf(
            self.remediation_complexity.universe,
            [7,
            10,
            10]
        ...
        self.rules = [
            ctrl.Rule(
                self.violation_probability['low'] & self.business_impact['low'],
                self.risk_score['very_low']
            ),
            ctrl.Rule(
                self.violation_probability['low'] & self.business_impact['medium'],
                self.risk_score['low']
            ),
            ctrl.Rule(
                self.violation_probability['low'] & self.business_impact['high'],
                self.risk_score['medium']
            ),
            ctrl.Rule(
                self.violation_probability['medium'] & self.business_impact['low'],
                self.risk_score['low']
            ),
            ctrl.Rule(
                self.violation_probability['medium'] & self.business_impact['medium'],
                self.risk_score['medium']
            ),
            ctrl.Rule(
                self.violation_probability['medium'] & self.business_impact['high'],
                self.risk_score['high']
            ),
            ctrl.Rule(
                self.violation_probability['high'] & self.business_impact['low'],
                self.risk_score['medium']
            ),
            ctrl.Rule(
                self.violation_probability['high'] & self.business_impact['medium'],
                self.risk_score['high']
            ),
            ctrl.Rule(
                self.violation_probability['high'] & self.business_impact['high'],
                self.risk_score['very_high']
            ),
        ]
    ...
    def predict_with_confidence(self, features: np.ndarray,
        ...
        try:
            ...
            if 'lstm' in self.models and
                self.models['lstm'] is not None and sequence_data is not None:
                ...
                with torch.no_grad():
                    ...
                    predictions['lstm'] = (
                        lstm_proba[:,
                        1] > 0.5).astype(int) if lstm_proba.shape[1] > 1 else (lstm_proba > 0.5).astype(int
                    )
                    probabilities['lstm'] = lstm_proba[:, 1] if lstm_proba.shape[1] > 1 else lstm_proba.flatten()

            # Prophet prediction
            if 'prophet' in self.models and
                self.models['prophet'] is not None and prophet_data is not None:
...
class CompliancePredictor:
    ...
    async def predict_compliance(self, resource_data: Dict[str, Any],
        ...
        try:
            ...
            final_prediction = (
                ensemble_results['probabilities'][0] if len(ensemble_results['probabilities']) > 0 else 0.5
            )
            confidence_score = (
                ensemble_results['confidence_scores'][0] if len(ensemble_results['confidence_scores']) > 0 else 0.0
            )
            ...
            remediation_complexity = self._calculate_remediation_complexity(
                resource_data,
                drift_results
            )
            ...
            return {
                ...
                'ensemble_details': {
                    ...
                    'individual_probabilities': ensemble_results.get(
    ...
    def _categorize_violations(
        ...
        categories = {
            'security': {
                'probability': violation_prob * 0.8 if drift_results.get('drift_class') = (
                    = 'significant' else violation_prob * 0.3,
                )
