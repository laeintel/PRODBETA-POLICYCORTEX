...
class CostOptimizationModel:
    ...
    def _extract_cost_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        cost_features = pd.DataFrame({
            ...
            'cost_volatility_7': cost_col.rolling(
                7,
                min_periods=1).std() / cost_col.rolling(7,
                min_periods=1).mean(
            ),
    ...
    def _extract_utilization_features(
        self,
        data: pd.DataFrame,
        ...
        for col in utilization_cols:
            if col in data.columns:
                ...
                utilization_features[f'{col}_rolling_mean_7'] = util_col.rolling(
                    7,
                    min_periods=1).mean(
                )
                utilization_features[f'{col}_rolling_std_7'] = util_col.rolling(
                    7,
                    min_periods=1).std(
                )
                utilization_features[f'{col}_rolling_max_7'] = util_col.rolling(
                    7,
                    min_periods=1).max(
                )
                utilization_features[f'{col}_rolling_min_7'] = util_col.rolling(
                    7,
                    min_periods=1).min(
                )
                utilization_features[f'{col}_rolling_median_7'] = util_col.rolling(
                    7,
                    min_periods=1).median(
                )

                # Percentiles
                utilization_features[f'{col}_p95_7'] = util_col.rolling(
                    7,
                    min_periods=1).quantile(0.95
                )
                utilization_features[f'{col}_p50_7'] = util_col.rolling(
                    7,
                    min_periods=1).quantile(0.50
                )
                ...
                # Efficiency metrics
                utilization_features[f'{col}_efficiency'] = util_col / (
                    util_col.rolling(30,
                    min_periods=1).max() + 1e-6
                )
    ...
    def _extract_sku_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'sku' in data.columns:
            ...
            sku_features['is_compute_optimized'] = data['sku'].str.contains(
                'F',
                case=False).astype(int
            )
            sku_features['is_memory_optimized'] = data['sku'].str.contains(
                'M|E',
                case=False).astype(int
            )
            sku_features['is_general_purpose'] = data['sku'].str.contains(
                'D|A',
                case=False).astype(int
            )
    ...
    def _extract_workload_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'response_time' in data.columns:
            ...
            workload_features['response_time_p95_7'] = rt_col.rolling(
                7,
                min_periods=1).quantile(0.95
            )
    ...
    def _extract_economic_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'timestamp' in data.columns:
            ...
            economic_features['holiday_effect'] = (
                ((timestamp.dt.month == 12) | (timestamp.dt.month == 1)).astype(int)
            )
    ...
    async def _train_prophet_models(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        for resource_type in data['resource_type'].unique():
            ...
            daily_costs = (
                type_data.groupby(type_data['timestamp'].dt.date)['cost'].sum().reset_index()
            )
    ...
    async def _identify_rightsizing_opportunities(
        ...
        if (cpu_util < thresholds['cpu_utilization_low'] and
            ...
            if recommended_sku:
                ...
                if (current_cost - optimized_cost) / current_cost > = (
                    thresholds['savings_threshold']:
                )
    ...
    async def _identify_scheduling_opportunities(
        self,
        resource: Dict[str,
        Any]
    ) -> List[OptimizationOpportunity]:
    ...
    async def _identify_spot_instance_opportunities(
        self,
        resource: Dict[str,
        Any]
    ) -> List[OptimizationOpportunity]:
        ...
        if resource.get('fault_tolerant', False) and
            resource.get('resource_type') == 'virtual_machine':
            current_cost = resource.get('monthly_cost', 0)
