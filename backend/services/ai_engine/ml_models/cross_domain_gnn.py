...
class GovernanceGraphBuilder:
    ...
    def build_graph(self, governance_data: Dict[str, Any]) -> HeteroData:
        ...
        try:
            ...
            logger.info(f"Built governance graph with {len(graph.node_types)} node types and {len(graph.edge_types)} edge types")
            return graph
    ...
    def _add_resource_nodes(self, graph: HeteroData, resources: List[Dict]):
        ...
        for resource in resources:
            try:
                ...
                logger.warning(f"Error extracting features for resource {resource.get('id', 'unknown')}: {e}")
                continue
...
class CorrelationEngine:
    ...
    def _get_graph_statistics(self, graph: HeteroData) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'total_nodes': sum(graph[node_type].x.size(0) if hasattr(graph[node_type], 'x') and graph[node_type].x is not None else 0 for node_type in graph.node_types),
            'total_edges': sum(graph[edge_type].edge_index.size(1) if hasattr(graph[edge_type], 'edge_index') else 0 for edge_type in graph.edge_types)
        }
