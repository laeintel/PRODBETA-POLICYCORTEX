...
CONVERSATION_REQUESTS = Counter(
    "conversation_requests_total", "Total conversation requests", ["intent", "domain", "status"]
)
INTENT_CLASSIFICATION_DURATION = Histogram(
    "intent_classification_duration_seconds", "Intent classification duration"
)
POLICY_SYNTHESIS_DURATION = Histogram(
    "policy_synthesis_duration_seconds", "Policy synthesis duration"
)
CONVERSATION_ACCURACY = Gauge(
    "conversation_accuracy", "Conversation understanding accuracy", ["domain"]
)
...
class ConversationConfig:
    ...
    governance_domains: List[str] = field(
        default_factory=lambda: ["security", "compliance", "cost", "performance", "operations"]
    )
...
class GovernanceEntityExtractor:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            "cost_metrics": [r"\$\d+(?:,\d{3})*(?:\.\d{2})?", r"\b\d+\s*(dollars?|USD|cents?)\b"],
        }
...
class PolicySynthesizer:
    ...
    def _calculate_policy_confidence(self, policy_text: str) -> float:
        ...
        found_keywords = sum(
            1 for keyword in governance_keywords if keyword.lower() in policy_text.lower()
        )
...
class QueryTranslator:
    ...
    def translate_query(
        self, intent: GovernanceIntent, entities: Dict[str, Any], query_text: str
    ) -> Dict[str, Any]:
        """Translate natural language query to API call specification"""

        if intent not in self.api_mappings:
            return {"error": f"No API mapping for intent: {intent}", "success": False}
        ...
        api_call = {
            ...
            "parameters": {},
            "headers": {"Content-Type": "application/json", "X-Query-Source": "conversational-ai"},
        }
    ...
    def _calculate_translation_confidence(self, entities: Dict[str, Any], query_text: str) -> float:
        ...
        ambiguity_penalty = sum(
            0.05 for phrase in ambiguous_phrases if phrase in query_text.lower()
        )
...
class ResponseGenerator:
    ...
    def generate_response(
        self, query_result: Dict[str, Any], context: ConversationContext
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            elif context.current_intent in [
                GovernanceIntent.SECURITY_ANALYSIS,
                GovernanceIntent.COST_OPTIMIZATION,
            ]:
    ...
    def _generate_policy_response(
        ...
        if count == 0:
            ...
            response = template.format(
                count=count, domain=context.current_domain or "governance", policy_list=policy_list
            )
    ...
    def _generate_compliance_response(
        ...
        response = template.format(standard=standard, status=status, details=details_text)
    ...
    def _generate_generic_response(
        ...
        return {"response": response, "response_type": "generic", "data": data, "success": True}
    ...
    def _generate_error_response(
        ...
        return {"response": response, "response_type": "error", "success": False}
...
class ConversationalGovernanceIntelligence:
    ...
    def __init__(self, config: ConversationConfig):
        ...
        self.intent_classifier = IntentClassifier(
            model_name=config.nlu_model_name, num_intents=len(GovernanceIntent), config=config
        ).to(self.device)
        ...
        logger.info(
            "Conversational Governance Intelligence initialized",
    ...
    async def process_conversation(
        ...
        try:
            ...
            logger.error("Conversation processing failed", session_id=session_id, error=str(e))
    ...
    async def _classify_intent(
        ...
        predicted_domain = (
            self.config.governance_domains[domain_idx]
            if domain_idx < len(self.config.governance_domains)
            else None
        )

        # Log metrics
        CONVERSATION_REQUESTS.labels(
            intent=predicted_intent.value, domain=predicted_domain or "unknown", status="success"
        ).inc()
    ...
    async def _handle_policy_creation(
        self, user_input: str, context: ConversationContext
    ) -> Dict[str, Any]:
        ...
        with POLICY_SYNTHESIS_DURATION.time():
            ...
            # Synthesize policy
            policy_result = self.policy_synthesizer.synthesize_policy(policy_description, domain)
    ...
    async def _handle_governance_query(
        self, user_input: str, context: ConversationContext
    ) -> Dict[str, Any]:
        ...
        translation_result = self.query_translator.translate_query(
            context.current_intent, context.extracted_entities, user_input
        )
        ...
        query_result = await self._execute_simulated_query(translation_result["api_call"], context)

        # Generate response
        response_result = self.response_generator.generate_response(query_result, context)
    ...
    async def _execute_simulated_query(
        self, api_call: Dict[str, Any], context: ConversationContext
    ) -> Dict[str, Any]:
        ...
        if "/policies" in endpoint:
            ...
            return {"success": True, "data": {"items": ["Item 1", "Item 2", "Item 3"]}}
    ...
    def _check_clarification_needed(
        self, context: ConversationContext, entities: Dict[str, Any]
    ) -> Optional[List[str]]:
    ...
    async def _handle_clarification(
        self, missing_info: List[str], context: ConversationContext
    ) -> Dict[str, Any]:
        """Handle clarification requests"""

        missing_text = " and ".join(missing_info)
        response = (
            f"I need more information to help you better. Could you please specify {missing_text}?"
        )
...
def create_conversational_intelligence(
    config: Optional[ConversationConfig] = None,
) -> ConversationalGovernanceIntelligence:
