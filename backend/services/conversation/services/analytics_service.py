...
class AnalyticsService:
    ...
    async def _update_counters(
        ...
        try:
            ...
            # Hourly counters
            hourly_key = (
                f"counter:hourly:{timestamp.strftime('%Y-%m-%d-%H')}:{event_type}:{category}"
            )
        ...
        try:
            ...
            if satisfaction_score is not None:
                ...
                if current_avg and current_count:
                    ...
                    current_count = int(current_count)
                    new_avg = (
                        (current_avg * current_count + satisfaction_score) / (current_count + 1)
                    )
                    new_count = current_count + 1
                else:
    ...
    async def get_user_analytics(
        ...
        try:
            ...
            if user_analytics["satisfaction_scores"]:
                user_analytics["avg_satisfaction"] = (
                    sum(user_analytics["satisfaction_scores"]) / len(user_analytics["satisfaction_scores"])
                )
            else:
