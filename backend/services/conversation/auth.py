...
class AuthManager:
    ...
    async def check_conversation_permission(self, user_info: Dict[str, Any], required_permission: str) -> bool:
    ...
    async def get_user_conversation_quota(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "limits": limits,
                "remaining": {
                    "daily_conversations": max(0, limits["daily_conversation_limit"] - quota_info["daily_conversations"]),
                    "monthly_conversations": max(0, limits["monthly_conversation_limit"] - quota_info["monthly_conversations"]),
                    "daily_messages": max(0, limits["daily_message_limit"] - quota_info["daily_messages"]),
                    "monthly_messages": max(0, limits["monthly_message_limit"] - quota_info["monthly_messages"])
                }
            }
    ...
    async def increment_conversation_quota(self, user_id: str, increment_type: str) -> bool:
        ...
        try:
            ...
            if increment_type == "conversation":
                if (usage.get("daily_conversations", 0) >= limits.get("daily_conversation_limit", 50) or
                    usage.get("monthly_conversations", 0) >= limits.get("monthly_conversation_limit", 1000)):
                    return False
                ...
                if (usage.get("daily_messages", 0) >= limits.get("daily_message_limit", 500) or
                    usage.get("monthly_messages", 0) >= limits.get("monthly_message_limit", 10000)):
                    return False
