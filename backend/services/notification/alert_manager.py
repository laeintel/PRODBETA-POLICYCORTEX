...
class AlertManager:
    ...
    async def create_escalation_rule(self,
                                   rule_name: str,
                                   alert_conditions: Dict[str, Any],
                                   escalation_levels: List[Dict[str, Any]]) -> str:
        """
    ...
    async def _should_suppress_alert(self, alert: Alert) -> bool:
        ...
        for existing_alert in self.alerts.values():
            if (existing_alert.title == alert.title and
                ...
                existing_alert.status in [AlertStatus.OPEN, AlertStatus.ACKNOWLEDGED] and
                (alert.created_at - existing_alert.created_at) < duplicate_threshold):
                return True
    ...
    async def _correlate_alert(self, alert: Alert) -> Optional[str]:
        ...
        for existing_alert in self.alerts.values():
            if (existing_alert.tenant_id == alert.tenant_id and
                ...
                existing_alert.status in [AlertStatus.OPEN, AlertStatus.ACKNOWLEDGED] and
                (alert.created_at - existing_alert.created_at) < correlation_window):
    ...
    async def _matches_escalation_conditions(self,
                                           alert: Alert,
    ...
    async def _send_channel_notification(self,
                                       alert: Alert,
                                       channel: NotificationChannel,
                                       level: Optional[EscalationLevel] = None) -> None:
    ...
    async def _run_escalation(self, alert: Alert, rule: EscalationRule) -> None:
        ...
        try:
            for i, level_config in enumerate(rule.escalation_levels[1:], 1):  # Skip level 0 (already sent)
    ...
    async def _load_default_channels(self) -> None:
        ...
        await self.create_notification_channel(
            "Primary Email",
            "email",
            {"recipients": ["oncall@policycortex.com"]}
        )
        
        await self.create_notification_channel(
            "Manager Email",
            "email",
            {"recipients": ["manager@policycortex.com"]}
        )
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        for status in AlertStatus:
            status_counts[status.value] = sum(
                1 for a in self.alerts.values() if a.status == status
            )
        ...
        for source in AlertSource:
            source_counts[source.value] = sum(
                1 for a in self.alerts.values() if a.source == source
            )
