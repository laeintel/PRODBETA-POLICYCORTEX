...
class AlertManager:
    ...
    async def _schedule_escalation(self, alert_id: str, escalation_rule: Dict[str, Any]) -> None:
        ...
        try:
            escalation_time = (
                datetime.utcnow() + timedelta(minutes=escalation_rule["delay_minutes"])
            )
                ...
                try:
                    ...
                    for alert_id in active_alerts:
                        try:
                            ...
                            logger.error(
                                "auto_resolution_processing_failed",
                                error=str(e),
                                alert_id=alert_id
                            )
                            continue
                ...
                try:
                    ...
                    if state == AlertState.ACTIVE.value:
                        ...
                        if alert.get("resolved_at"):
                            ...
                            resolution_time = (
                                (resolved_at - created_at).total_seconds() / 3600  # hours
                            )
                    ...
                    stats["alerts_by_severity"][severity] = stats["alerts_by_severity"].get(
                        severity,
                        0
                    ) + 1
