            ...
            for delivery_type in ["email", "sms", "push", "webhook"]:
                ...
                if delivery_data:
                    ...
                    return DeliveryStatus(
                        ...
                        status=DeliveryStatusEnum.DELIVERED if delivery.get(
                            "delivered_count",
                            0
                        ) > 0 else DeliveryStatusEnum.FAILED,
                        sent_at=datetime.fromisoformat(delivery["sent_at"]),
                        delivered_at=datetime.fromisoformat(
                            delivery["sent_at"]) if delivery.get("delivered_count",
                            0
                        ) > 0 else None,
                        failed_at=datetime.fromisoformat(
                            delivery["sent_at"]) if delivery.get("failed_count",
                            0
                        ) > 0 else None,
                        retry_count=0,
