...
class WebhookService:
    ...
    async def send_webhook(self, request: WebhookRequest) -> NotificationResponse:
        ...
        try:
            ...
            status = (
                DeliveryStatusEnum.DELIVERED if result["success"] else DeliveryStatusEnum.FAILED
            )
            message = result.get(
                "message",
                "Webhook sent successfully" if result["success"] else "Webhook failed"
            )
                ...
                try:
                    ...
                    if delivery_data:
                        ...
                        for detail in delivery.get("delivery_details", []):
                            ...
                            if detail.get("error"):
                                error = detail["error"]
                                stats["failure_reasons"][error] = stats["failure_reasons"].get(
                                    error,
                                    0
                                ) + 1
    ...
    async def get_webhook_subscriptions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for subscription_id in subscription_ids:
                subscription_data = (
                    await self.redis_client.get(f"webhook_subscription:{subscription_id}")
                )
        ...
        try:
            ...
            for subscription_id in subscription_ids:
                try:
                    ...
                    asyncio.create_task(
                        self._send_subscription_webhook(subscription,
                        webhook_payload)
                    )

                except Exception as e:
                    logger.error(
                        "webhook_event_trigger_failed",
                        error=str(e),
                        subscription_id=subscription_id
                    )
                    continue
    ...
    async def _send_subscription_webhook(
        ...
        try:
            ...
            logger.error(
                "subscription_webhook_send_failed",
                error=str(e),
